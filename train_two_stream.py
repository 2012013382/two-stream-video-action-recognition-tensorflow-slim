...
TRAIN_LOG_DIR = os.path.join('../log/train/two_stream/', time.strftime('%Y-%m-%d-%H:%M:%S', time.localtime(time.time())))
...
def train_fusion():
	...
	optimizer = slim.learning.create_train_op(two_stream_loss, opt)#, variables_to_train = variables_for_training)
		...
		flow_acc_epoch = 0
		    ...
		    best_epoch = epoch
		    saver.save(sess, TRAIN_CHECK_POINT + 'two_stream_trained.ckpt')
		...
		if best_flow_acc < flow_acc_epoch / v_step:
                ...
                print('Validation best epoch {}, best acc {}, best ls {}, best rgb acc {}, best flow acc {}, loss {}, acc {}, rgb_acc {}, flow_acc {}'.format(best_epoch + 1, best_val_acc, best_val_ls, best_rgb_acc, best_flow_acc, ls_epoch / v_step, acc_epoch / v_step, rgb_acc_epoch / v_step, flow_acc_epoch / v_step))
